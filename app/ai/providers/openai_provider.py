...
class OpenAILLMProvider:
    def __init__(self, api_key: str = None, model_name: str = "gpt-4o-mini"):
        ...
        self.llm = ChatOpenAI(
            model_name=model_name, openai_api_key=api_key, temperature=1
        )
...
class IOpenAISpeechProvider:
    def __init__(
        ...
        self.memory = ChatMessageHistory()
        self.llm = ChatOpenAI(
            model_name=model_config["tts"], openai_api_key=api_key, temperature=1
        )
        self.client = AsyncOpenAI(api_key=api_key)

    async def generate_tts(
        self, text: str, voice: str = "alloy", language: str = "pt_BR"
    ) -> BytesIO:
